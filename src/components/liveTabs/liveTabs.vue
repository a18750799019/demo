<template>
  <div class="liveTabs">
    <div class="liveTabsIn"
            ...
            <div :key="index"
              ...
              <div class="rightContent inline-block">
                ...
                <div class="optBtns">
                  ...
                  <a v-bind:class="{ generated :item.quakeLevel>=6 }"
                     @click.stop="showReport(item, '灾情简报')">灾情简报</a>
                  <a v-bind:class="{ generated :item.quakeLevel>=6 }"
                     @click.stop="showReport(item, '决策报告')">决策报告</a>
                </div>
                <img src="imgs/xinfeng.png"
        ...
        <el-tab-pane label="地址查询"
          ...
          <div class="listPanel">
            <div v-if="hasSearch && !addressObj.list.length"
                 style="color:#fff;"
                 class="margin-top-20 text-center">查询信息</div>
            <div v-for="(item,i) in addressObj.list"
                 class="infoItem"
                 :key="i"
      ...
      <distor-statis ref="distorStatis"
                     @close="close"
                     v-if="isshowDialog"></distor-statis>
    </div>

    <div class="leftCtl"
         :class="{'leftCtlSwit':!leftCtl}"
         @click="switchLeft"></div>
  </div>
</template>
<script>
