<template>
  <div class="liveTabs">
    <div class="liveTabsIn" v-if="leftCtl">
      ...
      >
        <el-tab-pane label="查询" name="first" class="subTab">
          <div class="searchPanle" style="height:20px;">
            <!-- <el-form>
          ...
          <div class="listPanel" style="height:calc(100vh - 210px);">
            <div
              :key="index"
              v-for="(item, index) in quakeObj.list"
              :class="{ on: $store.state.disasterStatistics == item }"
              class="infoItem position-relative"
              @click="setCente(item, 1)"
            >
              <div class="leftIcon inline-block">
                <div class="icon">
                  {{ index + (quakeObj.currPage - 1) * quakeObj.pageSize + 1 }}
                </div>
              </div>
              <div class="centerContent inline-block">
                <div class="subContent" :style="item.styleObj">
                  {{ item.quakeLevel ? item.quakeLevel + "" : "-" }}
                </div>
                <!-- <div class="subContent">{{item.depth?item.depth+'Km':"-"}}</div> -->
              </div>
              <div class="rightContent inline-block">
                <div>
                  <span class="ellipsis-one eaTitle" :title="item.address"
                    >{{ item.address }}
                  </span>
                  <span style="color:red" v-if="item.isSimulates == '1'"
                    >&nbsp;[模拟]</span
                  >
                </div>
                <div class="ellipsis-one">{{ item.seismicTime }}</div>
                <div class="ellipsis-one">
                  {{ "纬度:" + item.lat + "° /经度:" + item.lon + "°" }}
                </div>
                <div class="optBtns">
                  <!-- v-if="item.areal==1" -->
                  <a @click.stop="showStatis(item)">灾情统计</a>
                  <a
                    v-bind:class="{ generated: item.quakeLevel >= 6 }"
                    @click.stop="showReport(item, '灾情简报')"
                    >灾情简报</a
                  >
                  <a
                    v-bind:class="{ generated: item.quakeLevel >= 6 }"
                    @click.stop="showReport(item, '决策报告')"
                    >决策报告</a
                  >
                </div>
                <img
                  src="imgs/xinfeng.png"
                  v-if="item.isReported == 1"
                  title="已有上报数据"
        ...
        <el-tab-pane label="地址查询" name="second" class="subTab">
          ...
          <div class="listPanel">
            <div
              v-if="hasSearch && !addressObj.list.length"
              style="color:#fff;"
            ...
            <div
              v-for="(item, i) in addressObj.list"
              class="infoItem"
              :key="i"
              @click="setCente(item, 2)"
            >
              <div class="leftIcon inline-block">
              ...
              <div class="rightContent2 inline-block">
                ...
                <div class="adressRow ellipsis-one">
                  地址:{{ item.address }}
                </div>
              </div>
            </div>
...
export default {
  ...
  mounted() {
    this.initYearsOpts();
    this.citiesList.list = this.$config.areaRange.concat(
      this.$areacode.province[0].city
    );
    this.getListInfo()
  },
  methods: {
    ...
    getListInfo(params = {}) {
      ...
      let _p = {};
      //debugger
      this.conditionObj[0].check=false
      this.conditionObj[1].check=true
      if (this.conditionObj.every(v => v.check)) {
      } else {
        this.conditionObj.filter(v => {
          // v.check = item.type==v.type && item.check;
      ...
      let p2 = {},
        _r = {};
      if (
        (_r = this.$config.areaRange.find(v => v.name == this.citiesList.curr))
      ) {
        p2["areal"] = _r.value;
      } else {
          ...
          this.quakeObj.list = data.rows.filter(v => {
            if (Number.parseFloat(v.quakeLevel)) {
              this.$config.earthquakeLevelType.filter(x => {
                if (
                  Number.parseFloat(v.quakeLevel) >= x.min &&
                  Number.parseFloat(v.quakeLevel) <= x.max
                ) {
