<template>
  <div class="leftSubPanel"
    ...
    <template v-if="type =='liveMode'">
      <img class="close cursor-pointer"
    ...
    <div v-if="type=='liveMode' && activeName=='first'"
         style="height:100%;">
      <div class="location">
        <div class="left inline-block"><img src="/imgs/icon-curr.png"
               alt=""></div>
        ...
        <div style="display:flex;justify-content: space-evenly;margin-top:10px;    flex-wrap: wrap;">
          <el-button size="mini"
                     @click="distribute('intensity')"
                     :disabled="liveDetailsInner.length==0"
                     class="font-size-12 padding-vertical-5 cursor-pointer">上报震感分布</el-button>
          <el-button size="mini"
                     @click="distribute('casualties')"
                     :disabled="liveDetailsInner.length==0"
                     class="font-size-12 padding-vertical-5 cursor-pointer">上报伤亡分布</el-button>
          <el-button size="mini"
                     @click="distribute('damage')"
                     :disabled="liveDetailsInner.length==0"
                     class="font-size-12 padding-vertical-5 cursor-pointer"
                     style="margin-top:10px;margin-left:0">上报建筑物损失分布</el-button>
        </div>
      </div>
      <div class="detailPanel">
        <div class="liveDetail"
             v-for="item in liveDetailsInner">
          <div class="headtabs">
            <div class="headItem"
                 :class="{'on':item.selectedItem == 1}"
                 @click="item.selectedItem=1">上报详情</div>
            <div class="headItem centerItem"
                 :class="{'on':item.selectedItem == 2}"
                 @click="item.selectedItem=2">现场图片</div>
            <div class="headItem"
                 :class="{'on':item.selectedItem == 3}"
                 @click="item.selectedItem=3">现场视频</div>
          </div>
          ...
          <div v-if="item.selectedItem == 1"
            ...
            <table>
              <tr>
                ...
                <td>
                  <span class="btnPhone"
                        @click="callPhone(item.user ?item.user.account:item.reportUserPhone,item.user ?item.user.contact:item.reportUserPhone)"><img src="/imgs/icon-phone.png">电话</span>
                  <span v-if="false"
                        class="btnMessage"
                        @click="sendMesg(item)"><img src="/imgs/icon-message.png">消息</span>
                </td>
              </tr>
              <tr>
          ...
          <div class="smallSubPanel2"
               v-if="item.selectedItem == 2">
          ...
          <div class="smallSubPanel3"
               v-if="item.selectedItem == 3">
            <video controls
    ...
    <div class="addressModePanel"
         v-if="type =='addressMode'">
      <div v-for="item in searchResult"
        ...
        <div class="resRightContent">
          ...
          <div>{{`距离选中位置约${getGreatCircleDistance(item.lat,item.lng,centerLat,centerLng)}公里`}}</div>
          <div>{{`所处生命线:${item.type}`}}</div>
        </div>
      </div>
      <div v-if="searchResult.length%10==0"
           class="moreLiveLine"
           @click="showMoveLiveLine">更多</div>
    </div>
    <div class="legend"
         v-if="casualtyItem"
         style="position: fixed;bottom: 161px;right: 230px;white-space: nowrap;">
      <iframe :src="casualtyItem.legendurl"
              style="background:#fff;width:220px;height:200px;"
