<template>
  <div class="ds-main-container"
    ...
    <div class="header">
      ...
      <div class="right">
        <ul>
          <li @click="$store.commit('toggleMessageShow')">
            <img :src="'/imgs/list.jpg'"
                 width="25px">
            <div>信息查询</div>
          </li>
          <li @click="$store.commit('toggleRainsearchShow')">
    ...
    <dialog-panel width="941px"
      ...
      <div style="height:518px;padding:0 10px;">
        <form-config :tableData="dialogReportStatisObj.tableData"
                     :tableConf="dialogReportStatisObj.tableConf">
        </form-config>
      </div>
    </dialog-panel>
    <dialog-panel class="mockDialog"
      ...
      <el-form ref="form"
        ...
        <el-form-item label="地震发生地点:"
                      style="padding-top:16px;">
          <el-row :gutter="5">
            <el-col :span="6">
              <el-select v-model="form.city"
                         placeholder="市"
                         @change="changeAreacode($event,'mockCity')"
                         class="citySelect">
                <el-option v-for="item in form.opt_cities"
                           :key="item.areacode"
                           :label="item.name"
                           :value="item.areacode">
                </el-option>
            ...
            <el-col :span="6">
              <el-select v-model="form.town"
                         placeholder="县区"
                         @change="changeAreacode($event,'mockTown')"
                         class="countrySelect">
                <el-option v-for="item in form.opt_towns"
                           :key="item.areacode"
                           :label="item.name"
                           :value="item.areacode">
                </el-option>
          ...
          <el-row :gutter="5"
            ...
            <el-col :span="8"
                    class="longitude">
              经度&nbsp;&nbsp;<el-input oninput="value=value.replace(/[^\d.]/g,'')"
                        style="width:calc(100% - 55px)"
                        class="longitudeInput"
                        placeholder="输入经度"
                        v-model="form.lng"></el-input>
            </el-col>
            <el-col :span="8"
                    class="latitude">
              &nbsp;&nbsp;&nbsp;纬度&nbsp;&nbsp;<el-input oninput="value=value.replace(/[^\d.]/g,'')"
                        style="width:calc(100% - 50px)"
                        class="latitudeInput"
                        placeholder="输入纬度"
                        v-model="form.lat"></el-input>
            </el-col>
            <el-col :span="6">
              <el-button class="mapSelect"
                         type="primary"
                         @click="showMap()">地图选点</el-button>
            </el-col>
          </el-row>
        </el-form-item>
        <el-form-item label="地震震源震级:"
                      style="padding-top:8px;">
          <el-row :gutter="5">
            <el-col :span="8"
                    class="magnitude">
              震源&nbsp;&nbsp;<el-input oninput="value=value.replace(/[^\d.]/g,'')"
                        style="width:calc(100% - 50px)"
                        placeholder="输入震源"
                        v-model="form.range"
                        class="magnitudeInput"></el-input>
            </el-col>
            <el-col :span="2"
                    class="kilometer">
              千米
            ...
            <el-col :span="8"
                    class="focus">
              震级&nbsp;&nbsp;<el-input oninput="value=value.replace(/[^\d.]/g,'')"
                        style="width:calc(100% - 50px)"
                        placeholder="输入震级"
                        v-model="form.level"
                        class="focusInput"></el-input>
            </el-col>
            <el-col :span="2"
                    class="level">
              级
      ...
      <div style="text-align:center;margin-bottom:10px;padding-top:12px;">
        <el-button @click="mockEa()"
                   class="mockEaBtn"
                   style="color:#fff;width:120px;height:36px;">模拟地震</el-button>
      </div>
    </dialog-panel>

    <dialog-panel :dialogVisible="repwsd.state"
                  :title="repwsd.title"
                  :width="repwsd.width"
      ...
      <el-form ref="form"
               :model="repwsd"
               label-width="100px"
               style="height:300px;margin:0 50px 0 0;">
          ...
          <el-row>
            <el-col>
              <el-input type="password"
                        placeholder="请输入密码"
                        v-model="repwsd.oldpwd"></el-input>
            </el-col>
          </el-row>
        </el-form-item>
        <el-form-item label="新密码:"
          ...
          <el-input type="password"
                    placeholder="请输入密码"
                    v-model="repwsd.newpwd"></el-input>
        </el-form-item>
        <el-form-item label="确认密码:"
                      style="padding-top:16px;">
          <el-input type="password"
                    placeholder="请输入密码"
                    v-model="repwsd.renewpwd"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <mapDialog @mapClickInfo="mapClickInfoReceive"
               @closeMapObj="closeMapObjReceive"
               :dialogStatisObj="{state:isshowMapChoose}"></mapDialog>
    <keep-alive>
  ...
  methods: {
    mockEa () {
      ...
      try {
        ...
        commonApi.pagingPost(this.$api.mockea_one, obj1).then(res => {
          if (res.code == 200) {
