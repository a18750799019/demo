<template>
  <div class="sysContainer">
    <!-- <div class="backMap" @click="goBackMain">返回地图</div> -->
            ...
            <div class="inline-block userForm" style="padding-top:5px;">
              <form-config 
                ...
                &nbsp;&nbsp;<el-button class="search" @click="userOperateHandle(null,'search')">查询</el-button>
                      <el-button class="search" @click="handleClear()">清空</el-button>
                </div>
              </form-config>
            </div>
...
import formConfig from "@/_sys_components/formConfigOld"
...
export default {
    ...
    return {
      ...
      dialogSysUserObj:{
        rules:{
          ...
          pwd:[
              { validator: (rule, value, callback)=>{
              if (this.dialogSysUserObj.opt==="edit") {
                callback();
              }else if (value === '') {
          ...
          repwd: [
            { validator: (rule, value, callback)=>{
              if (this.dialogSysUserObj.opt==="edit") {
                ...
                callback(new Error('请再次输入密码'));
              } else if (value !== this.dialogSysUserObj.pwd) {
                callback(new Error('两次输入密码不一致!'));
              } else {
                callback();
              }
  ...
  methods:{
    handleClear(){
      this.areacode.cityLabel=''
      this.areacode.townLabel=''
      this.areacode.deptLabel=''
      this.areacode.jobsLabel=''
      this.areacode.name=''
    },
    ...
    addSubmitSysUser(){
      this.$refs.dialogSysUserObjRuleForm.validate((valid) => {
        ...
        let _pwd={}
        if(this.dialogSysUserObj.pwd != ""){
          _pwd = {password:pwdEncrypt(this.dialogSysUserObj.pwd,this.dialogSysUserObj.pwd.slice(0,18))}
        }
    ...
    goBackMain(){
      ...
      for(var i=0; i<this.$parent.rightMenu.length;i++){
        this.$parent.rightMenu[i].seleted = false
      }
    }
  }
}
